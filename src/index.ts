 ...
 *
 * Multi-user reservation sniper with Discord webhook notifications and Supabase backend.
 *
...
import { initializeNotifier, getNotifier } from "./notifications/webhook";
...
main().catch((error) => {
  logger.error({ error: String(error) }, "Fatal error starting application");
  process.exit(1);
});
